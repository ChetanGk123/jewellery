...
- **2.6 partially fixed** — the product page now has a "Write a Review" form (signed-in customers, one review per product, lands `pending` for the existing moderation queue). Code-complete and browser-verified, but its migration (`submit_review` RPC) is **not yet applied to production** — awaiting confirmation. See `TASKS.md` 4.15.
