...
- **2.6 fixed** — the product page now has a "Write a Review" form, gated to customers with a `Delivered` order for that product (user-flagged mid-session: the initial version had no purchase check), one review per product, lands `pending` for the existing moderation queue. Both `submit_review` migrations applied + verified via SQL probes and live browser checks. Noted in passing: several products' `rating`/`review_count` header stats were stale seed numbers that didn't match their actual (much shorter) review lists — a pre-existing trigger corrects this automatically the next time any review is written for that product; not something 4.15 needs to fix directly. See `TASKS.md` 4.15.
- **2.4 fixed** — `sitemap.ts`, `robots.ts`, `metadataBase`, a branded default OG image, and Organization + Product JSON-LD all added. See `TASKS.md` 4.16 for the full verification trail, including a self-caught bug where an early version silently dropped the default OG image for every product without real photography (nearly the whole catalog).
- **3.1 fixed** — `/shop` and category pages now use a dedicated paginated query (count-then-clamp-then-range) instead of an unbounded one, with a Prev/numbered/Next pager. Two real bugs (an out-of-range page crashing with a PostgREST 500, and a missing join breaking the category-page count query) were found and fixed during verification — see `TASKS.md` 4.17.
