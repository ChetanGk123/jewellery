...
- **2.6 fixed** — the product page now has a "Write a Review" form (signed-in customers, one review per product, lands `pending` for the existing moderation queue). `submit_review` RPC applied + verified via SQL probes and a live browser submission. Noted in passing: several products' `rating`/`review_count` header stats were stale seed numbers that didn't match their actual (much shorter) review lists — a pre-existing trigger corrects this automatically the next time any review is written for that product; not something 4.15 needs to fix directly. See `TASKS.md` 4.15.
