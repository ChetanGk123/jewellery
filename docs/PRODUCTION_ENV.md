...
| `HANDBOOK_ORIGIN` | **In production, no `/docs` route** — it falls through to the storefront's category catch-all ("Category not found", HTTP 200). Set it to the Mintlify deployment origin to reverse-proxy the operator handbook onto this domain (`next.config.ts` `rewrites()`). **Also requires the Mintlify dashboard's "Host at" set to `<domain>/docs`** — that mode is what moves its assets to `/mintlify-assets/*` + `/_mintlify/*` and its pages under `/docs`. In dev (unset) `/docs` instead **307-redirects** to `http://localhost:3333` (`bun run handbook`) — `mint dev` emits root-relative links, so it can only be linked to, not proxied. See `handbook/README.md`. |
