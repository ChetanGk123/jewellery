 ...
 */
export async function upsertCoupon(input: CouponInput): Promise<CouponActionResult> {
  ...
  const expiresAt = input.expiresAt?.trim() ? `${input.expiresAt.trim()}T18:29:59.000Z` : null

  // Max discount only caps percent coupons; ignore it for fixed/free-shipping.
  const maxDiscountPaise = input.kind === "percent" ? rupeesToPaise(input.maxDiscount) : null
...
export async function toggleCoupon(id: string, active: boolean): Promise<CouponActionResult> {
  await requireAdmin(ROUTES.adminCoupons)
