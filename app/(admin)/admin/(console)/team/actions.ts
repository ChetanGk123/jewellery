...
function messageFor(code: string | undefined, raw: string): TeamActionResult {
  if (raw.includes("NO_ACCOUNT"))
    return {
      ok: false,
      error:
        "No account exists for that email yet — ask them to sign up first, then grant access.",
    };
  if (raw.includes("ALREADY_ADMIN"))
    return { ok: true, notice: "That person is already an admin." };
  ...
  if (raw.includes("NOT_AN_ADMIN"))
    return { ok: false, error: "That person isn't an admin." };
