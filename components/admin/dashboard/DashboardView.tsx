  ...
  return (
    <div className={`${CARD} flex flex-col gap-3 p-5`}>
      <div className="flex items-center justify-between">
        <span className="text-[12px] font-medium tracking-[0.04em] text-[#8A7E74]">{label}</span>
        <span
      ...
      <span className="text-[32px] font-semibold leading-none text-[#2A1F1A]">{value}</span>
      {foot}
    </div>
  )
}
...
function Delta({ pct }: { pct: number | null }) {
  if (pct === null) {
    return <span className="text-[12px] font-medium text-[#A99C90]">No baseline yesterday</span>
  }
  const up = pct >= 0
  return (
    <span className="text-[12px] font-medium" style={{ color: up ? "#1B7A3D" : "#C0392F" }}>
...
function FootLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link href={href} className="text-[12px] font-medium text-maroon-700 hover:underline">
      {children}
    </Link>
  )
}
...
function RevenueChart({ bars, totalPaise }: { bars: RevenueBar[]; totalPaise: number }) {
  const peak = Math.max(0, ...bars.map((b) => b.revenuePaise))

  return (
    <div className={`${CARD} p-[22px]`}>
      <div className="mb-5 flex items-baseline justify-between">
        <span className="text-[15px] font-semibold text-[#2A1F1A]">Revenue — last 7 days</span>
        <span className="text-[18px] font-semibold text-maroon-700">{formatPaise(totalPaise)}</span>
      </div>
      {bars.length === 0 ? (
        <p className="py-8 text-center text-[13px] text-[#A99C90]">No revenue data yet.</p>
      ) : (
        <div className="flex items-end gap-[14px]" style={{ height: CHART_HEIGHT }}>
          {bars.map((b, i) => {
            const h = peak > 0 ? Math.max(6, Math.round((b.revenuePaise / peak) * 128)) : 6
            return (
              <div key={i} className="flex h-full flex-1 flex-col items-center justify-end gap-2">
                ...
                />
                <span className="text-[11px] font-medium text-[#A99C90]">{b.day}</span>
              </div>
            )
          })}
        </div>
  ...
  return (
    <div className={`${CARD} overflow-hidden`}>
      <div className="flex items-center justify-between border-b border-[#EFE9DE] px-[22px] py-[18px]">
        <span className="text-[15px] font-semibold text-[#2A1F1A]">Recent Orders</span>
        <Link
      ...
      {orders.length === 0 ? (
        <p className="px-[22px] py-10 text-center text-[13px] text-[#A99C90]">No orders yet.</p>
      ) : (
        orders.map((o) => (
          <div
            key={o.orderNo}
  ...
  return (
    <div className={`${CARD} p-5`}>
      <div className="mb-4 flex items-center gap-2">
        ...
        <span className="text-[15px] font-semibold text-[#2A1F1A]">Low Stock Alerts</span>
      </div>
      {rows.length === 0 ? (
        <p className="py-4 text-center text-[13px] text-[#A99C90]">Everything is well stocked.</p>
      ) : (
            ...
            return (
              <div key={l.sku} className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate text-[13px] font-medium text-[#2A1F1A]">{l.name}</div>
...
function TopSellers({ rows }: { rows: TopSellerRow[] }) {
  return (
    <div className={`${CARD} p-5`}>
      <span className="text-[15px] font-semibold text-[#2A1F1A]">Top Sellers This Month</span>
      {rows.length === 0 ? (
        <p className="py-4 text-center text-[13px] text-[#A99C90]">No sales this month yet.</p>
      ) : (
        <div className="mt-4 flex flex-col gap-3.5">
          {rows.map((tp, i) => (
            <div key={`${tp.name}-${i}`} className="flex items-center gap-3">
              <Swatch />
              <div className="min-w-0 flex-1">
                <div className="truncate text-[13px] font-medium text-[#2A1F1A]">{tp.name}</div>
                <div className="mt-[3px] text-[11px] text-[#A99C90]">{tp.units} sold</div>
              </div>
