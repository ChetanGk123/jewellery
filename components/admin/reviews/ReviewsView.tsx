...
/**
 * Reviews moderation (TASKS 3.7, prototype-matched): Pending / Approved / All
 * filter pills with live counts over a responsive card grid. Each card shows the
 * product, star rating, title, body, author · date and a status pill; a pending
 * card carries ✓ Approve / × Reject actions that flip its status through the
 * `setReviewStatus` server action. The storefront shows approved reviews only
 * (already RLS-filtered). Lands on Pending — the moderation queue.
 */
export function ReviewsView({ reviews, counts }: Props) {
  const [filter, setFilter] = useState<ReviewFilter>("Pending");
  const [pendingId, setPendingId] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [, startTransition] = useTransition();

  const visible = reviews.filter((r) => matchesFilter(r.status, filter));
