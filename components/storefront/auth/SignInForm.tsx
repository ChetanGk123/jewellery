...
export function SignInForm({ next }: { next: string }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      {mode === "password" ? (
        <PasswordForm target={target} />
      ) : (
        <OtpForm target={target} />
      )}
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-[11px] uppercase tracking-[0.14em] text-[#9C8A84]">
          or
        </span>
        <span className="h-px flex-1 bg-[#EFE3D0]" />
      </div>
...
function OtpForm({ target }: { target: string }) {
  ...
  if (sentTo) {
    return (
      <form
        onSubmit={verify.handleSubmit(onVerify)}
        noValidate
        className="flex flex-col gap-4"
      >
  ...
  return (
    <form
      onSubmit={request.handleSubmit(onRequest)}
      noValidate
      className="flex flex-col gap-4"
    >
      <AuthField
