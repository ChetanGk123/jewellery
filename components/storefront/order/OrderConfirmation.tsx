 ...
 */
export function OrderConfirmation({ confirmation, couponDropped, emailSent }: Props) {
  return (
    <main className="mx-auto flex max-w-[680px] flex-col items-center gap-[18px] px-6 pb-[90px] pt-[70px] text-center">
      ...
      <p className="m-0 max-w-[460px] text-[16px] font-light leading-[1.6] text-[#5E4A44]">
        Your order <span className="font-semibold text-maroon-700">{confirmation.orderNo}</span> is
        confirmed.{" "}
        {emailSent ? (
          <>
          ...
          <>
            ...
            <span className="text-maroon-900">{confirmation.customerEmail}</span> to reach you about
            it.
          </>
        )}
      </p>
      ...
      {couponDropped && (
        <p className="m-0 max-w-[460px] rounded border border-[#E7C98A] bg-[#FBF3DE] px-4 py-3 text-[13px] leading-snug text-[#8A6D1E]">
          The coupon you entered was no longer valid, so it wasn't applied. Your order total below
      ...
      <dl className="mt-2 flex min-w-[300px] flex-col gap-2.5 rounded border border-[#E7D9C2] bg-[#FFFDF8] px-8 py-6 text-left">
        ...
        <Row label="Amount payable" value={formatPaise(confirmation.totalPaise)} emphasis />
