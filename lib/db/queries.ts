 ...
 */
export const STOREFRONT_VISIBLE_STATUSES = ["Active", "Low stock", "Out of stock"] as const
...
function pickPrimaryImage(images: EmbeddedImage[] | null | undefined): ProductImageBrief | null {
...
export const getCategoryTiles = unstable_cache(
  async (): Promise<CategoryTile[]> => {
    const [cats, counts] = await Promise.all([
      publicClient.from("category").select("*").order("sort_order", { ascending: true }),
      publicClient.from("product").select("category_id").in("status", STOREFRONT_VISIBLE_STATUSES),
    ])
