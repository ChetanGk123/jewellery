...
- ✅ **1.4 — Category page.** `app/category/[slug]/page.tsx` (flat route — `(storefront)` group deferred until admin lands in Phase 3; URLs identical): breadcrumb, title + product count, `ProductCard` grid, empty-collection state, and `generateMetadata` (name/description). Unknown slug → `notFound()` (verified **HTTP 404**); valid slug renders live products (verified 200 + browser @1440). New `getCategoryBySlug`; grid uses `getProducts({categorySlug})`. *Sort control + filter sidebar deferred to 1.6 (URL search-param state).*
